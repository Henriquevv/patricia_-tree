...
int _contador_no = 0;
int _contador_chave = 0;
Node* _raiz = NULL;
PatriciaTree* _arvore = NULL;

// INICIALIZAR A ARVORE
PatriciaTree* init_patricia_tree() {
  if(_arvore == NULL) {
    _raiz = _novo_no(NULL, 0, 0);
    _arvore = (PatriciaTree*)calloc(1, sizeof(PatriciaTree));
    _arvore->inserir = &inserir_arvore;
    _arvore->encontrar = &encontrar_arvore;
    _arvore->print = &rtree_print;
    _arvore->free = &rtree_free;
    _arvore->temchave = &temchave_arvore;
    _arvore->contadorchave = &contador_chave;
    return _arvore;
  } else {
    return NULL;
  }
...
void inserir_arvore(const char* chave, int valor) {
  _inserir(_raiz, chave, valor);
  _contador_chave++;
}


void _inserir(Node *n, const char *chave, int valor) {
  ...
  if (rel) {
    ...
    _strmtc(rel->prefixo, chave, match);
    if (strlen(match) == strlen(rel->prefixo)) {
      free((void *)match);
      _inserir(rel, (chave + strlen(rel->prefixo)), valor);
      return;
    }

    // Verifica se o prefixo é uma chave completa ou parte de uma
    if (strlen(match) > 0) {
      Node *paiComum = _novo_no(match, 0, 0);
      paiComum->nivel = n->nivel + 1;
      if(strlen(match) == strlen(chave)) {
        paiComum->ehNoFolha = 1;
        paiComum->valor = valor;
      } else {
        _add_filho(paiComum, (chave + strlen(match)), 1, valor);
      }
      _update_nivel(rel, rel->nivel + 1);
      char filho_prefixo[strlen(rel->prefixo)];
      strcpy(filho_prefixo, rel->prefixo);
      _update_prefixo(rel, (filho_prefixo + strlen(match)));
      _anexar_filho(paiComum, rel);
...
Node *_novo_no(const char *prefixo, int ehNoFolha, int valor) {
  ...
  _contador_no++; //Contador global de nós
  return n;
}

//Adiciona um novo nó filho ao nó pai
void _add_filho(Node *noPai, const char *prefixo, int ehNoFolha, int valor) {
  ...
  Node *filho = _novo_no(prefixo, ehNoFolha, valor);
...
void _anexar_filho(Node *noPai, Node *filho) {
...
const int* encontrar_arvore(const char *chave) {
  Node* n = _encontrar(_raiz, chave);
  return n ? &(n->valor) : NULL;
}

//Encontrar chave
Node* _encontrar(Node *n, const char *chave) {
  printf("> \"%s\"\n", chave);
  ...
  for (int i = 0; i < n->contadorFilho; i++) {
    ...
    if (strcmp(n->filhos[i]->prefixo, chave) == 0 &&
        n->filhos[i]->ehNoFolha == 1) {
      return n->filhos[i];
    } else {
      //Caso contrario, verifica se o filho do nó é igual a chave. Recursivamente
      if (_strcnt(chave, n->filhos[i]->prefixo)) {
        return _encontrar(n->filhos[i], (chave + strlen(n->filhos[i]->prefixo)));
      }
    }
  }
...
void _remover_no(Node *pai, Node *filho) {
    ...
    if (index != -1) {
        ...
        if (pai->contadorFilho == 1 && pai != _raiz && !pai->ehNoFolha) {
...
void remover_no_arvore(const char* chave) {
    ...
    Node *no_para_remover = _encontrar_no(_raiz, chave);
    //Se for encontrado e for um nó folha, remove
    if (no_para_remover != NULL && no_para_remover->ehNoFolha) {       
        _remover_no(_encontrar_pai(_raiz, no_para_remover), no_para_remover);
...
Node *_encontrar_pai(Node *raiz, Node *filho) {
  ...
  for (int i = 0; i < raiz->contadorFilho; i++) {
    if (raiz->filhos[i] == filho) {
      ...
      Node *pai = _encontrar_pai(raiz->filhos[i], filho);
      if (pai != NULL) {
        return pai;
      }
    }
...
Node* _encontrar_no(Node *n, const char *chave) {
    ...
    for (int i = 0; i < n->contadorFilho; i++) {
        if (strcmp(n->filhos[i]->prefixo, chave) == 0) {
            ...
            if (_strcnt(chave, n->filhos[i]->prefixo)) {
                return _encontrar_no(n->filhos[i], (chave + strlen(n->filhos[i]->prefixo)));
            }
        }
    }
    return NULL;
...
int temchave_arvore(const char* chave) {
  return encontrar_arvore(chave) ? 1 : 0;
}

// Retorna numero total de chaves na arvore
int contador_chave() {
  return _contador_chave;
}
...
void _update_nivel(Node *n, int nivel) {
  n->nivel = nivel;
  for (int i = 0; i < n->contadorFilho; i++) {
    _update_nivel(n->filhos[i], nivel + 1);
  }
}

//Atualiza o prefixo em comum
void _update_prefixo(Node *n, const char *prefixo) {
  free((void *)n->prefixo);
  n->prefixo = (char *)calloc(strlen(prefixo) + 1, sizeof(char));
  strcpy(n->prefixo, prefixo);
}
...
void _print_arvore(Node *n) {
  _print_no(n);
  for (int i = 0; i < n->contadorFilho; i++) {
    _print_arvore(n->filhos[i]);
  }
}

void _print_no(Node *n) {
